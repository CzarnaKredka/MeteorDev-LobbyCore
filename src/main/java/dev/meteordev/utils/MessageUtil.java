    ...
    public static void message(CommandSender sender, MessageType type, String message) {
        if (sender instanceof Player) {
            ...
            switch (type) {
                ...
                case TITLE_SUBTITLE:
                    String[] split = message.split("%NOWA_LINIA%");

                    if (split.length < 2) {
                        throw new IllegalArgumentException("Notice with TITLE_SUBTITLE needs to have %NOWA_LINIA% to include both title and subtitle.");
                    }
